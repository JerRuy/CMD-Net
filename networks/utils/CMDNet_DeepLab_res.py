...
from networks.utils.CMDNet_DeepLab_res_utils import resnet18
...
class ResNet_BasicBlock_OS8(nn.Module):
    def __init__(self, num_layers):
        ...
        self.chx3 = nn.Conv2d(filters[0] + filters[1], filters[1], 1, padding=0)
        self.chx3b = nn.BatchNorm2d(filters[1])
        self.chx3r = nn.ReLU(inplace=True)
        
        self.chx4 = nn.Conv2d(filters[1] + filters[2], filters[2], 1, padding=0)
        self.chx4b = nn.BatchNorm2d(filters[2])
    ...
    def forward(self, x):
        ...
        # pass x through (parts of) the pretrained ResNet:
        x2,x3 = self.resnet(x) # (shape: (batch_size, 128, h/8, w/8)) (it's called c3 since 8 == 2^3)



        x2d = self.maxpool2(x2)  
        #print(x2d.size()) 
        #print(x3.size()) 
        ...
        x3 = self.chx3b(x3)
        x3 = self.chx3r(x3)  
        x4 = self.layer4(x3) # (shape: (batch_size, 256, h/8, w/8))
        ...
        #x5 = self.layer4(x4)   
        #print(x4.size())     
        x5 = self.layer5(x4) # (shape: (batch_size, 512, h/8, w/8))
