...
def apply_weight_sharing2(model_dict,pparameter_mask, bits=5):
    """
    Applies weight sharing to the given model
    """
    print('apply_weight_sharing')
    model_dict_new={}
    for k,v in model_dict.items():
        print(type(k),type(v),v.size)
        if v.size>32:
            weight = v
            mask = pparameter_mask[k]
            mat = weight[mask]
            min_ = min(mat.data)
            max_ = max(mat.data)
            space = np.linspace(min_, max_, num=2 ** bits)
            ...
            new_weight = kmeans.cluster_centers_[kmeans.labels_].reshape(-1)
            # print(new_weight.shape, mask.shape)
            model_dict_new[k] = torch.from_numpy(new_weight.reshape(list(mask.shape)))
        else:
            model_dict_new[k]=model_dict[k]
    print('apply_weight_sharing over')
